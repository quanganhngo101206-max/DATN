        ...
        try {
            ...
            for (int i = 0; i < productDetailIds.size(); i++) {
                ...
                if (!productDetailIdSet.add(productDetailId)) {
                    throw new RuntimeException("Không được nhập trùng cùng một biến thể sản phẩm trong một phiếu!");
                }
                ...
                if (pd == null
                        ...
                        || pd.getProduct() == null) {
                    throw new RuntimeException("Biến thể sản phẩm không tồn tại hoặc đã bị xóa!");
                }

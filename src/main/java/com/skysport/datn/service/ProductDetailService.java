...
public class ProductDetailService {
    ...
    private void validate(ProductDetail detail) {
        ...
        if (detail.getMinStock() != null && detail.getMinStock() < 0) {
            throw new RuntimeException("Tồn kho tối thiểu không được nhỏ hơn 0");
        }

        if (detail.getMaxStock() != null && detail.getMaxStock() < 0) {
            throw new RuntimeException("Tồn kho tối đa không được nhỏ hơn 0");
        }

        if (detail.getMinStock() != null
                && detail.getMaxStock() != null
                && detail.getMaxStock() < detail.getMinStock()) {
            throw new RuntimeException("Tồn kho tối đa phải lớn hơn hoặc bằng tồn kho tối thiểu");
        }

        if (detail.getStatus() != null
                && detail.getStatus() != 0
                && detail.getStatus() != 1) {
            throw new RuntimeException("Trạng thái biến thể không hợp lệ");
        }
